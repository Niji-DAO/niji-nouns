...
task('nijinouns-deploy-and-configure', 'Deploy and configure all contracts')
  ...
  .setAction(async (args, { ethers, run }) => {
    ...
    const contracts = await run('nijinouns-deploy', args);
    ...
    await run('nijinouns-populate-descriptor', {
    ...
    await (await contracts.NounsDescriptorV2.instance.transferOwnership(executorAddress)).wait();
