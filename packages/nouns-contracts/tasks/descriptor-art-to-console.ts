...
task(
  ...
  .setAction(async ({ count, start, exportPath }, { ethers }) => {
    ...
    if (count !== undefined) {
      ...
      skills = skills.slice(start, count + start);
    }
    ...
    console.log('=== SKILLS ===\n');
    console.log(`skillsCompressed: '${skillsPage.encodedCompressed}'\n`);
    console.log(`skillsLength: ${skillsPage.originalLength}\n`);
    console.log(`skills count: ${skillsPage.itemCount}`);
    saveToFileAbiEncoded(path.join(exportPath, 'skillsPage.abi'), skillsPage);
  });
